...
class VideoDataset(data.Dataset):
    ...
    def make_weights_for_balanced_classes(self):
        ...
        for idx, item in enumerate(self.video_list):
            weight[idx] = weight_per_class[item.label]
            # weight[idx] = weight_per_class[item['target']]
        return weight
...
class ImageDataset(data.Dataset):
    ...
    def get(self, record):
        seg_imgs = [Image.open(record.path).convert('RGB')]

        images = self.transform(seg_imgs)
        images = torch.reshape(images, (-1, 3, self.image_size, self.image_size))
...
def train_data_loader():
    ...
    train_transforms = torchvision.transforms.Compose([GroupRandomSizedCrop(image_size),
                                                       ...
                                                       GroupRandomAffine(),
                                                       GroupRandomColor_jitter(),
                                                       Stack(),
                                                       ToTorchFormatTensor()])
    train_data = VideoDataset(list_file="./annotation/train.txt",
                              num_segments=8,
                              duration=2,
                              mode='train',
...
def test_data_loader():
    ...
    test_transform = torchvision.transforms.Compose([GroupResize(image_size),
                                                     Stack(),
                                                     ToTorchFormatTensor()])
    test_data = VideoDataset(list_file="./annotation/validation.txt",
                             num_segments=8,
                             duration=2,
                             mode='test',
...
def image_train_data_loader():
    image_size = 224
    train_transforms = torchvision.transforms.Compose([GroupRandomSizedCrop(image_size),
                                                       GroupRandomHorizontalFlip(),
                                                       Stack(),
                                                       ToTorchFormatTensor()])
    train_data = ImageDataset(list_file="./annotation/train.txt",
                              num_segments=8,
                              duration=2,
                              mode='train',
                              transform=train_transforms,
...
def image_test_data_loader():
    image_size = 224
    test_transform = torchvision.transforms.Compose([GroupResize(image_size),
                                                     Stack(),
                                                     ToTorchFormatTensor()])
    test_data = ImageDataset(list_file="./annotation/validation.txt",
                             num_segments=8,
                             duration=2,
                             mode='test',
                             transform=test_transform,
